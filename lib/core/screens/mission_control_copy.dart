...
final class MissionControlCopy {
  ...
  String get organizationHint => _t('p. ej. Homelab', 'e.g. Homelab', '例如：Homelab');
  ...
  String get deleteOrganizationBody => _t('Solo se elimina este espacio local. Los profiles de Hermes no cambian.', 'Only this local workspace is removed. Hermes profiles are not changed.', '只會移除這個本機工作區，Hermes 設定檔不會變更。');
  String get loading => _t('Leyendo el equipo…', 'Reading team state…', '正在讀取團隊狀態…');
  ...
  String get addToMissionControl => _t('Añadir a Bots', 'Add to Bots', '加入 Bots');
  String get createAgent => _t('Nuevo agente', 'New agent', '新增代理程式');
  String get createAgentDescription => _t('Crea un profile real de Hermes con su modelo y capacidades.', 'Create a real Hermes profile with its own model and capabilities.', '建立一個具備專屬模型及能力的真正 Hermes 個人檔案。');
  String get createRoom => _t('Crear sala', 'Create room', '建立房間');
  String get createRoomDescription => _t('Elige entre 2 y 6 bots y abre su sala de coordinación.', 'Choose 2–6 bots and open their coordination room.', '選擇 2–6 個 bots，開啟它們的協調房間。');
  ...
  String get noBots => _t('Un bot es un compañero con nombre propio, memoria, skills y chat propios. Crea el primero para empezar.', 'A bot is a named teammate with its own memory, skills and chat. Create the first one to get started.', 'Bot 是一位有名稱的隊友，擁有自己的記憶、技能和聊天功能。建立第一個 Bot 以開始使用。');
  String get botNeedsYou => _t('te necesita', 'needs you', '需要你');
  String get needMoreBots => _t('Crea otro bot antes de abrir una sala.', 'Create another bot before opening a room.', '開啟房間前，請先建立另一個 Bot。');
  String get needTwoAgents => _t('Crea al menos dos bots antes de abrir una sala de equipo.', 'Create at least two bots before opening a team room.', '開啟團隊房間前，請至少建立兩個 Bot。');
  ...
  String get botRosterUpdateFailed => _t('Hermes no pudo actualizar este bot.', 'Hermes did not update this bot.', 'Hermes 沒有更新此 Bot。');
  ...
  String agentCreated(String name) => _t('Bot @$name creado. Añádelo a una sala cuando quieras.', 'Bot @$name created. Add it to a room when you are ready.', 'Bot @$name 已建立。準備好後，將其加入房間。');
  ...
  String get roomPurposeHint => _t('p. ej. Mantener producción estable', 'e.g. Keep production stable', '例如保持正式環境穩定');
  String get roomNameInvalid => _t('Escribe un nombre después del símbolo #.', 'Enter a name after the # symbol.', '請在 # 符號後輸入名稱。');
  ...
  String get roomNoActivity => _t('Todavía no hay actividad publicada para esta sala.', 'No activity has been published for this room yet.', '此房間尚未發佈任何活動。');
  ...
  String get noRooms => _t('Crea una sala para empezar a hablar con tu equipo.', 'Create a room to start talking with your team.', '建立房間，開始與團隊交流。');
  String get openRoom => _t('Abrir sala', 'Open room', '開啟房間');
  String get linkedWork => _t('tareas enlazadas', 'linked tasks', '已連結的任務');
  String unavailableTaskLink(String boardId, String taskId) => _t('Tablero $boardId · $taskId · no cargada', 'Board $boardId · $taskId · not loaded', '看板 $boardId · $taskId · 未載入');
  String get unavailableLinkedWork =>
      _t('Trabajo enlazado no disponible', 'Linked work unavailable', '已連結的工作無法使用');
  String get roomContract => _t('El coordinador recibe tus mensajes y reparte el trabajo confirmado al equipo.', 'The coordinator receives your messages and assigns confirmed work to the team.', '協調員會接收你的訊息，並將已確認的工作分配給團隊。');
  String get deleteRoomTitle => _t('¿Eliminar sala?', 'Delete room?', '刪除房間？');
  String get deleteRoomBody => _t('Solo se elimina esta sala. Sus chats y tareas se conservan.', 'Only this room is removed. Its chats and tasks are kept.', '只會移除此房間。其聊天內容和任務會保留。');
  String get roomOperationPending => _t('Finaliza o recupera la tarea pendiente antes de editar o eliminar esta sala.', 'Finish or recover the pending Room task before editing or deleting this Room.', '編輯或刪除此房間前，請先完成或復原待處理的房間任務。');
  String get needsYou => _t('Necesita tu atención', 'Needs you', '需要你處理');
  String get usage => _t('Uso', 'Usage', '使用量');
  String get profilesUnavailable => _t('Esta instalación de Hermes no publica profiles.', 'This Hermes installation does not publish profiles.', '此 Hermes 安裝並不會發佈個人檔案。');
  String get roomsBrowseOnly => _t('Hermes no puede verificar el equipo ahora. Las salas guardadas siguen visibles en modo consulta.', 'Hermes cannot verify the team right now. Saved rooms remain visible in browse-only mode.', 'Hermes 目前無法驗證團隊。已儲存的房間仍會以僅供瀏覽模式顯示。');
  String get offline => _t('Hermes no está disponible. Los datos existentes del equipo siguen visibles.', 'Hermes is unavailable. Existing team data remains visible.', 'Hermes 無法使用。現有團隊資料仍會顯示。');
  String get staleData => _t('Algunos datos del equipo pueden estar desactualizados.', 'Some team data may be out of date.', '部分團隊資料可能已過時。');
  String get noProfiles => _t('No hay bots disponibles aquí.', 'No bots are available here.', '這裏沒有可用的 Bots。');
  String get noTasks =>
      _t('Todavía no hay tareas aquí.', 'There are no tasks here yet.', '這裡暫時沒有任務。');
  String get kanbanUnavailable => _t('El tablero de tareas no está disponible en esta instalación de Hermes.', 'The task board is not available on this Hermes installation.', '此 Hermes 安裝版本不提供任務看板。');
  String get noActivity => _t('Hermes no ha publicado actividad reciente para este ámbito.', 'Hermes has not published recent activity for this scope.', 'Hermes 尚未發佈此範圍的近期活動。');
  String get noApprovals => _t('No hay aprobaciones observadas pendientes.', 'No observed approvals need attention.', '沒有需要跟進的已觀察審批。');
  ...
  String get partialCost => _t('Cobertura parcial', 'Partial coverage', '部分涵蓋範圍');
  String get staleProfiles => _t('Algunos profiles guardados ya no existen. Edita la organización para actualizarla.', 'Some saved profiles no longer exist. Edit the organization to update it.', '部分已儲存的設定檔已不存在。編輯組織以更新。');
  ...
  String get botFaceFallbackHint => _t('La forma y el color solo se usan si el bot no tiene sprite.', 'Shape and color are only used when the bot has no sprite.', '當 Bot 沒有 sprite 時，才會使用形狀和顏色。');
  String get botSpriteLabel => 'Sprite';
  String get botSpriteHint => _t('El sprite se convierte en la imagen de este bot.', 'The sprite becomes this bot\\\'s picture.', 'Sprite 會成為此 bot 的圖片。');
  ...
  String get botSpriteEmpty =>
      _t('No hay sprites disponibles.', 'No sprites available.', '沒有可用的 Sprite。');
  String get botSpriteUnsupported => _t('Esta instalación de Hermes no admite sprites por profile.', 'This Hermes installation does not support profile sprites.', '此 Hermes 安裝不支援個人檔案 Sprite。');
  String get botEditorSaved => _t('Bot actualizado', 'Bot updated', 'Bot 已更新');
  String get botEditorSaveFailed => _t('Hermes no aplicó los cambios.', 'Hermes did not apply the changes.', 'Hermes 沒有套用變更。');

  // Creación de bots (paridad con CreateAgentDialog de Hermes Desktop).
  String get createAgentSubtitle => _t('Un compañero con nombre propio, memoria, skills y chat propios. Puede escribir a tus otros agentes.', 'A named teammate with its own memory, skills, and chat. It can message your other agents.', '一個具名隊友，擁有自己的記憶、技能和聊天功能。它可以向你的其他代理程式傳送訊息。');
  String get agentNameLabel => _t('Nombre', 'Name', '名稱');
  String get agentNameHint => 'inbox-triage';
  String get agentNameInvalid => _t('Usa minúsculas, números, guiones y guiones bajos.', 'Use lowercase letters, numbers, dashes and underscores.', '請使用小寫字母、數字、連字號和底線。');
  String get agentNameTaken => _t('Ya existe un agente con este nombre.', 'An agent with this name already exists.', '已經存在使用此名稱的代理程式。');
  ...
  String get agentDescriptionHint => _t('¿En qué debería ayudar este bot?', 'What should this bot help with?', '此 bot 應該協助處理甚麼？');
  String get modelInherited => _t('Heredado del profile de arranque', 'Inherited from the launch profile', '繼承自啟動設定檔');
  String get modelPickerTitle => _t('Elegir modelo', 'Choose model', '選擇模型');
  String get modelCatalogEmpty => _t('Esta instalación de Hermes no publicó un catálogo de modelos. Escribe proveedor y modelo a mano.', 'This Hermes installation did not publish a model catalog. Enter provider and model manually.', '此 Hermes 安裝沒有發布模型目錄。請手動輸入供應商及模型。');
  ...
  String get cloneFresh => _t('Profile nuevo (skills incluidas)', 'Fresh profile (bundled skills)', '全新設定檔（內置技能）');
  String get shareAuthLabel => _t('Compartir claves y cuentas con el profile principal', 'Share keys & accounts with the main profile', '與主要設定檔共用金鑰及帳戶');
  String get shareAuthHint => _t('Suscripciones, logins OAuth y API keys quedan compartidos (no copiados), así que los refrescos de token nunca se invalidan entre sí. Desmárcalo para una copia aislada.', 'Subscriptions, OAuth logins, and API keys stay shared (not copied), so token refreshes never invalidate each other. Uncheck for an isolated snapshot copy.', '訂閱、OAuth 登入及 API 金鑰會保持共用（不會複製），因此權杖重新整理不會令彼此失效。取消剔選即可建立獨立的快照副本。');
  String get noSkillsLabel => _t('Crear vacío (sin skills incluidas)', 'Create empty (skip bundled skills)', '建立空白（略過內置技能）');
  String get soulOptionalLabel =>
      _t('SOUL.md (opcional)', 'SOUL.md (optional)', 'SOUL.md（可選）');
  String get soulOptionalHint => _t('Déjalo en blanco para autogenerarla a partir de nombre, título y descripción.', 'Leave blank to auto-generate from name/title/description.', '留空即可根據名稱／標題／描述自動產生。');
  String get skillsLoading => _t('Cargando skills…', 'Loading skills…', '正在載入技能…');
  String get skillsUnavailable => _t('El catálogo de skills necesita un gateway más reciente (actualiza Hermes y reinícialo).', 'The skill catalog needs a newer gateway (update Hermes and restart it).', '技能目錄需要較新的閘道（請更新 Hermes 並重新啟動）。');
  String skillsFromSource(String source) => _t('Catálogo de $source: las skills desmarcadas se desactivan tras la creación.', 'Catalog from $source — unchecked skills are disabled after creation.', '目錄來源：$source — 未剔選的技能會在建立後停用。');
  String get createAgentSubmit => _t('Crear agente', 'Create agent', '建立代理人');
  String createAgentError(String detail) => _t('No se pudo crear el agente: $detail', 'Could not create the agent: $detail', '無法建立代理程式：$detail');
  ...
  String status(String value) => switch (value) {
    ...
    'approvalRequired' =>
      _t('Aprobación requerida', 'Approval required', '需要批准'),
    'error' => 'Error',
    _ => unknown,
  };
